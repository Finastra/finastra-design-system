...
StyleDictionary.registerFormat({
  ...
  formatter({ dictionary, file, options }) {
    ...
    return (
      ...
      `$tokens: (\n` +
      formattedVariables({ format: 'sass', dictionary, outputReferences }).replace(regex, '').replaceAll(';', ',') +
      `\n)\n`
    );
  }
});
...
const getStyleDictionaryConfig = (filename, source, include) => ({
  source, // build the special formats
  include,
  platforms: {
    css: {
      transforms: ['attribute/cti', 'name/cti/kebab', 'size/px', 'color/css', 'color/gradient/css'],
      buildPath: 'dist/css/',
      files: [
        {
...
for (const { filename, source } of themes) {
  StyleDictionary.extend(getStyleDictionaryConfig(filename, source, undefined)).buildAllPlatforms();
}

// Building sizes
const sizeFiles = await globby('../../packages/fds-tokens/size/*');
for (const file of sizeFiles) {
  StyleDictionary.extend(getStyleDictionaryConfig('spacing', [file], undefined)).buildAllPlatforms();
}

// Building typography
const typoFiles = await globby('../../packages/fds-tokens/typography/*');
for (const file of typoFiles) {
  StyleDictionary.extend(getStyleDictionaryConfig('typography', [file], undefined)).buildAllPlatforms();
}
