...
describe('data table', () => {
  describe(' - Accessibility', () => {
    it('loads accessibly', async () => {
      const el: DataTable = await fixture(html`<fds-data-table .dataSource=${ELEMENT_DATA} .columns=${dataTableColumns} .columnsToDisplay=${dataTableColumnsToDisplay}></fds-data-table>`);

      await elementUpdated(el);
      await expect(el).to.be.accessible();
    });
    it('loads accessibly', async () => {
      const el: DataTablePagination = await fixture(html`<fds-data-table-pagination .length=${demoLength} .pageIndex=${demoPageIndex} .pageSize=${demoPageSize} .pageSizeOptions=${demoPageSizeOptions} .showFirstLastButtons=${showFirstLastButtons}></fds-data-table-pagination>`);
