...
describe('Charts', () => {
  ...
  it('should disableSelection', async () => {
    const el: PieChart = await fixture(html`<fds-pie-chart legendPosition="top" legendHorizontalAlign="center" width="300px" height="300px" theme="dark" color="semantic-1" disable-selection  .data=${demoData} .labels=${demoLabels} ></fds-pie-chart>`);
  ...
  it('should change theme', async () => {
    const el: PieChart = await fixture(html`<fds-pie-chart theme="dark" color="categorical" .data=${demoData} .labels=${demoLabels} ></fds-pie-chart>`);
    await elementUpdated(el);    
    await expect(el.getColor()).to.not.be.empty
    el.color = "semantic-2"
    await expect(el.getColor()).to.not.be.empty
  ...
  it('loads accessibly - LineChart', async () => {
    const lineChart: LineChart = await fixture(html`<fds-line-chart  .series=${demoSeries} .xaxis=${demoXaxis} .yaxis=${demoYaxis} ></fds-pie-chart>`);
  ...
  it('should disable data labels - LineChart', async () => {
    const lineChart: LineChart = await fixture(html`<fds-line-chart hide-data-label .series=${demoSeries} .xaxis=${demoXaxis} .yaxis=${demoYaxis} ></fds-line-chart>`);
    await elementUpdated(lineChart);    
